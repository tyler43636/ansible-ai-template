...
```
.
├── ansible.cfg              # Ansible configuration (inventory, roles path, become settings)
...
└── .mcp.json                # Ansible MCP server configuration
```
...
This project has an Ansible MCP server configured in `.mcp.json`. Use the `mcp__ansible__*` tools when available:
